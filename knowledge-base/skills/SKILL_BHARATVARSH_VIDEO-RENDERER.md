...
Present prompts to the user for approval. If using the AI image generation flow (`--images` not provided), generate images via DALL-E 3 at 1024×1792. Save to `video-production/public/content/{post-slug}/images/`.
...
```bash
cd video-production
npx remotion render bhv-reel-{post-slug} --output ../content-pipelines/bharatvarsh/posts/{post_id}/{post_id}_reel.mp4
```

### Landscape Trailer (16:9 — 1920×1080)

```bash
npx remotion render bhv-trailer-{post-slug} --output ../content-pipelines/bharatvarsh/posts/{post_id}/{post_id}_trailer.mp4
```
...
Create the output directory at `content-pipelines/bharatvarsh/posts/{post_id}/video/` containing:

```
{post_id}_reel.mp4              — Instagram Reels / YouTube Shorts (9:16)
{post_id}_trailer.mp4           — Landscape trailer (16:9)
{post_id}_video_captions.txt    — Platform-specific caption variants
{post_id}_video_render_log.md   — Render specs and metadata
```
...
```
Video rendered for [post_id]: [topic]

Outputs:
  Reel (9:16): content-pipelines/bharatvarsh/posts/{post_id}/video/{post_id}_reel.mp4
  Trailer (16:9): content-pipelines/bharatvarsh/posts/{post_id}/video/{post_id}_trailer.mp4
  Captions: content-pipelines/bharatvarsh/posts/{post_id}/video/{post_id}_video_captions.txt

Duration: {X}s ({N} slides + intro + end card)
Next pending post: [next_post_id] or "Pipeline clear"
```
