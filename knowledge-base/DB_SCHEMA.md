...
| 40 | `creative_projects` | 0 | 18 | Creative Writer | 019 |
| 41 | `creative_project_steps` | 0 | 10 | Creative Writer | 019 |
| 42 | `brainstorm_sessions` | 0 | 12 | Creative Writer | 019 |
| 43 | `writing_outputs` | 0 | 17 | Creative Writer | 019 |
...
**Migration 019 (Creative Writer — written, not applied):**
- **Enums:** creative_project_type (novel, comic_series, blog_series, screenplay, poetry_collection, anthology, custom), creative_project_status (ideation, outlining, drafting, revising, editing, complete, paused, abandoned), brainstorm_method (socratic, constraint, character_driven, theme_exploration, genre_fusion, yes_and, mind_map), writing_output_type (blog, article, headline, slogan, caption, poem, script, chapter, scene, vignette, essay, ad_copy, social_post, custom)
- **Tables:** creative_projects (long-running creative work with Truby's 22-step structure, character webs, moral arguments, world-building), creative_project_steps (Truby's 22-step tracker, auto-populated for narrative types), brainstorm_sessions (7 ideation methods, standalone or project-linked), writing_outputs (versioned append-only drafts with parent chain)
- **Triggers:** moddatetime on creative_projects, brainstorm_sessions. writing_outputs is append-only (no updated_at).
- **Indexes:** slug unique on creative_projects, project_id + step_number unique on steps, project_id on sessions, project_id + output_type + version on outputs, parent_output_id on outputs
...
## Creative Writer Tables (Migration 019)

4 tables for creative writing projects, story structure, brainstorming, and versioned outputs.

### Table: `creative_projects`
_Core project table for long-running creative work. Supports Truby's 22-step story structure, character webs, moral arguments, and world-building._
...
| `project_type` | `creative_project_type` | NO |  | novel, comic_series, blog_series, screenplay, poetry_collection, anthology, custom |
| `status` | `creative_project_status` | NO | `'ideation'::creative_project_status` | ideation, outlining, drafting, revising, editing, complete, paused, abandoned |
| `universe` | `text` | YES |  | 'bharatvarsh' = load lore context |
| `genre` | `text[]` | YES |  | Multi-genre array |
| `logline` | `text` | YES |  | One-sentence premise |
| `synopsis` | `text` | YES |  | Longer description |
| `target_word_count` | `integer` | YES |  | Target total words |
| `current_word_count` | `integer` | NO | `0` | Auto-updated from writing_outputs |
| `moral_argument` | `jsonb` | YES |  | Truby's moral argument framework |
| `character_web` | `jsonb` | YES |  | Truby's character web |
| `world_rules` | `jsonb` | YES |  | Custom world-building data |
| `style_guide` | `jsonb` | YES |  | Discovered/created style conventions |
| `domain_id` | `uuid` | YES |  | FK → `life_domains.id` (SET NULL). Life Graph link |
| `tags` | `text[]` | YES | `'{}'` |  |
| `metadata` | `jsonb` | YES | `{}` |  |
| `created_at` | `timestamp with time zone` | NO | `now()` |  |
| `updated_at` | `timestamp with time zone` | NO | `now()` |  |
...
_Truby's 22-step story structure tracker. Auto-populated for narrative types (novel, comic_series, screenplay, anthology)._

| Column | Type | Nullable | Default | Notes |
|--------|------|----------|---------|-------|
| `id` | `uuid` | NO | `uuid_generate_v4()` | PK |
| `project_id` | `uuid` | NO |  | FK → `creative_projects.id` (CASCADE) |
| `step_number` | `integer` | NO |  | 1-22 Truby, 100+ custom |
| `step_name` | `text` | NO |  |  |
| `step_type` | `text` | NO | `'truby'` | 'truby' or 'custom' |
| `status` | `text` | NO | `'pending'` | pending, active, complete, skipped |
| `description` | `text` | YES |  | How this step applies to the project |
| `content` | `text` | YES |  | Written content/decisions |
| `decisions` | `jsonb` | NO | `'[]'::jsonb` | Key decisions made |
| `created_at` | `timestamp with time zone` | NO | `now()` |  |
| **Constraints** | | | | UNIQUE(`project_id`, `step_number`) |

### Table: `brainstorm_sessions`
_Interactive ideation sessions with 7 methods. Can be standalone or project-linked._

| Column | Type | Nullable | Default | Notes |
|--------|------|----------|---------|-------|
| `id` | `uuid` | NO | `uuid_generate_v4()` | PK |
| `project_id` | `uuid` | YES |  | FK → `creative_projects.id` (SET NULL). Standalone if null |
| `title` | `text` | NO |  |  |
| `method` | `brainstorm_method` | NO |  | socratic, constraint, character_driven, theme_exploration, genre_fusion, yes_and, mind_map |
| `prompt` | `text` | YES |  | Seed question/constraint |
| `ideas` | `jsonb` | NO | `'[]'::jsonb` | [{idea, score, notes, is_selected, source}] |
| `selected_ideas` | `jsonb` | NO | `'[]'::jsonb` | Promoted after evaluation |
| `constraints` | `jsonb` | NO | `'[]'::jsonb` | For constraint method |
| `conclusion` | `text` | YES |  | Final synthesis |
| `tags` | `text[]` | YES | `'{}'` |  |
| `metadata` | `jsonb` | YES | `{}` |  |
| `created_at` | `timestamp with time zone` | NO | `now()` |  |
| `updated_at` | `timestamp with time zone` | NO | `now()` |  |
...
_Versioned writing drafts. Append-only — new versions create new rows linked via parent_output_id._

| Column | Type | Nullable | Default | Notes |
|--------|------|----------|---------|-------|
| `id` | `uuid` | NO | `uuid_generate_v4()` | PK |
| `project_id` | `uuid` | YES |  | FK → `creative_projects.id` (SET NULL). Quick-mode = null |
| `step_id` | `uuid` | YES |  | FK → `creative_project_steps.id` (SET NULL). Which Truby step this serves |
| `output_type` | `writing_output_type` | NO |  | blog, article, headline, slogan, caption, poem, script, chapter, scene, vignette, essay, ad_copy, social_post, custom |
| `title` | `text` | NO |  |  |
| `content` | `text` | NO |  | Full draft |
| `version` | `integer` | NO | `1` |  |
| `word_count` | `integer` | YES |  |  |
| `platform` | `text` | YES |  | Target platform |
| `tone` | `text` | YES |  |  |
| `audience` | `text` | YES |  |  |
| `drive_file_id` | `text` | YES |  | If exported to Drive |
| `drive_url` | `text` | YES |  | If exported to Drive |
| `parent_output_id` | `uuid` | YES |  | FK → `writing_outputs.id` (SET NULL). Version chain |
| `tags` | `text[]` | YES | `'{}'` |  |
| `metadata` | `jsonb` | YES | `{}` |  |
| `created_at` | `timestamp with time zone` | NO | `now()` | No updated_at — append-only |
...
| `creative_projects_updated_at` | `creative_projects` | BEFORE | UPDATE | `EXECUTE FUNCTION moddatetime('updated_at')` |
| `brainstorm_sessions_updated_at` | `brainstorm_sessions` | BEFORE | UPDATE | `EXECUTE FUNCTION moddatetime('updated_at')` |

All `updated_at` triggers use `moddatetime()` from the `moddatetime` extension — automatically sets the column to `NOW()` on any `UPDATE`. Note: `writing_outputs` has no `updated_at` trigger — it is append-only (new versions create new rows).
...
| `creative_projects` | `domain_id` | `life_domains` | `id` | SET NULL |
| `creative_project_steps` | `project_id` | `creative_projects` | `id` | CASCADE |
| `brainstorm_sessions` | `project_id` | `creative_projects` | `id` | SET NULL |
| `writing_outputs` | `project_id` | `creative_projects` | `id` | SET NULL |
| `writing_outputs` | `step_id` | `creative_project_steps` | `id` | SET NULL |
| `writing_outputs` | `parent_output_id` | `writing_outputs` | `id` | SET NULL |
...
- **`creative_project_type`**: `novel`, `comic_series`, `blog_series`, `screenplay`, `poetry_collection`, `anthology`, `custom`
- **`creative_project_status`**: `ideation`, `outlining`, `drafting`, `revising`, `editing`, `complete`, `paused`, `abandoned`
- **`brainstorm_method`**: `socratic`, `constraint`, `character_driven`, `theme_exploration`, `genre_fusion`, `yes_and`, `mind_map`
- **`writing_output_type`**: `blog`, `article`, `headline`, `slogan`, `caption`, `poem`, `script`, `chapter`, `scene`, `vignette`, `essay`, `ad_copy`, `social_post`, `custom`
