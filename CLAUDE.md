...
The primary interface is a Claude.ai project (Category A) with 29 skills, 3 connectors (Gmail, Calendar, Drive), and a rich knowledge base. This Claude Code workspace handles what can't happen inside chat: terminal operations, multi-file code generation, git management, deployment, and MCP server development.
...
- **Tier 2 — Unified MCP Gateway:** ONE FastAPI Cloud Run service (`mcp-servers/ai-os-gateway/`) with 80 tools (15 modules). PostgreSQL (6), Google Tasks (9), Drive Write (3), Drive Read (3), Calendar Sync (3), Telegram (5), Life Graph (8), Capture (3), Contacts (8), Bharatvarsh (8), Composite (3), Media Gen (5), LinkedIn (4), Meta (4), Creative Writer (8). Telegram bot webhook also hosted on Gateway. Scales to zero. $0-7/month. Dashboard PWA (`dashboard/`) on separate Cloud Run service.
...
- **Database:** Cloud SQL PostgreSQL (pgvector, ltree) on shared bharatvarsh-db instance (bharatvarsh-website:us-central1:bharatvarsh-db). Database: ai_os. User: ai_os_admin. 45 tables in codebase (38 live) across 12 schema domains. Migrations 001-019 all applied.
...
- **MCP:** Gmail, Calendar, Drive (Tier 1 connectors). AI OS Gateway deployed on Cloud Run with 80 tools (15 modules): PostgreSQL (6), Google Tasks (9), Drive Write (3), Drive Read (3), Calendar Sync (3), Telegram (5), Life Graph (8), Capture (3), Contacts (8), Bharatvarsh (8), Composite (3), Media Gen (5), LinkedIn (4), Meta (4), Creative Writer (8). 80 tools / 15 modules total in codebase (56/10 deployed). Telegram bot @AsrAiOsbot webhook on Gateway with capture commands (/j, /e, /ei, /em, /img). Evernote, n8n (Tier 3 local STDIO, to configure).
...
│   └── skills/
├── .claude/skills/           ← Claude Code auto-discovered skills (29 skills)
...
├── mcp-servers/
│   └── ai-os-gateway/        ← Unified MCP Gateway (80 tools, 15 modules — 56/10 deployed)
│       ├── app/
│       │   ├── main.py
│       │   ├── config.py
│       │   ├── modules/      ← postgres.py, google_tasks.py, drive_write.py, drive_read.py, calendar_sync.py, telegram.py, life_graph.py, capture.py, contacts.py, bharatvarsh.py, composite.py, media_gen.py, linkedin.py, meta.py, creative_writer.py
...
│   └── creative-writing-plugin/ ← Creative writer plugin (skill, context, 8 MCP tools)
├── infra/
└── docs/
```

## Active Projects
1. **AI Operating System** — Sprint 12: Creative Writing Plugin + Social Connectors (BUILT). creative_writer.py (8 tools), linkedin.py (4 tools), meta.py (4 tools), migrations 016-019 applied, seed 014 applied, Cowork plugin with Truby 22-step framework, 7 brainstorm methods, 4 modes. Sprint 11b: ASR Visual Studio Cowork plugin (18 files, 3 skills, 3 engine modules, hybrid local+MCP rendering). 80 tools (15 modules), 45 tables in codebase, 29 skills, Dashboard PWA (9 pages, 28 components), Telegram Bot (10 commands), 891 contacts imported. Life Graph: 11 domains (001-011, 009 archived), 14 life_domains rows, Google Tasks aligned. Cowork plugin directory added.
...
## Current Sprint
Sprint 12 — Creative Writing Plugin + Social Connectors (BUILT). creative_writer.py (8 tools), linkedin.py (4 tools), meta.py (4 tools), migrations 016-019 all applied, seed 014 applied, Cowork plugin at cowork-configuration/creative-writing-plugin/ with Truby 22-step framework, 7 brainstorm methods, 4 modes (quick/project/brainstorm/critique). Sprint 11b — ASR Visual Studio Cowork Plugin (BUILT). Sprint 11 — Visual Content + Composite Queries (BUILT). Sprint 10-B — Bharatvarsh Lore (DEPLOYED). Sprint 10-A — Contact Intelligence (DEPLOYED). Codebase: 80 tools / 15 modules / 45 tables / 29 skills. Deployed: 56 tools / 10 modules / 38 tables.
