...
> **Infrastructure Update:** The Remotion video engine has been consolidated into `video-production/`. All Bharatvarsh video compositions now live at `video-production/src/projects/bharatvarsh/`. The unified system shares brand tokens, common components, and a single Remotion 4.0.438 install across all projects. The brand configuration is at `video-production/projects/bharatvarsh/project.yaml`.
...
**Determine the post number** — count the 1-indexed position of this row among all non-empty data rows in the CSV (skip header row and blank rows). This becomes the canonical `post-N` identifier used for all asset paths. For example, the 5th non-empty data row is `post-5`.
...
```
Next pending post: [post_id]  (post-[N] in the pipeline)
Topic: [topic]
...
node scripts/render_post.js \
  ...
  --output-dir "content-pipelines/bharatvarsh/rendered/post-1/"
```

**`--post-number` is required** — always pass it. The output directory and all output filenames use `post-{N}` (e.g., `post-1-instagram.png`, not `{post_id}_instagram.png`).
...
Save each caption as `post-{N}-{platform}-caption.txt` in the output directory (`content-pipelines/bharatvarsh/rendered/post-{N}/`).

Also create a combined `post-{N}-all-captions.txt` with all platforms separated by clear headers.
...
```
post-{N}-package.zip
├── post-{N}-instagram.png
├── post-{N}-instagram-caption.txt
├── post-{N}-twitter.png
├── post-{N}-twitter-caption.txt
├── post-{N}-linkedin.png
├── post-{N}-linkedin-caption.txt
├── post-{N}-facebook.png          (if facebook in platforms)
├── post-{N}-facebook-caption.txt  (if facebook in platforms)
├── post-{N}-all-captions.txt
└── post-{N}-render-manifest.json
```

All files are stored under `content-pipelines/bharatvarsh/rendered/post-{N}/` — no subdirectories, no BHV ID in the filename. The post_id is preserved inside the render manifest JSON only.
