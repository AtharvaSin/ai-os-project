---
name: drive-knowledge-distill
description: "Monthly knowledge distillation from Google Drive changes. Use when user says /drive-knowledge-distill, distill Drive knowledge, monthly knowledge sync, what changed in Drive, or knowledge review."
---
...
- Says `/drive-knowledge-distill`, "monthly knowledge sync", "distill Drive knowledge", "what changed in Drive", or "knowledge review"
- Asks to update the knowledge layer from recent Drive activity
- Requests a monthly knowledge curation cycle

Typical cadence: once per month, or ad-hoc when significant new documents land in Drive.
...
Test: call `list_drive_files(folder_path="AI OS/Knowledge/System/")`. If it returns file listings, use Mode A for all steps.

**Mode B — Pipeline DB Entries**
Test: call `query_db(sql="SELECT COUNT(*) FROM knowledge_ingestion_jobs WHERE status='success' AND completed_at > NOW() - INTERVAL '7 days'")`. If recent ingestion jobs exist, use drive_scan_state + knowledge_entries tables for discovery.

**Mode C — Manual Paste (fallback)**
If neither A nor B is available, ask the user to paste file contents or describe what changed. Process manually.
...
For each approved entry, call `insert_record` with:
...
For rejected entries, note the rejection in the conversation but do NOT store rejection state in the database. The file will be re-proposed on the next run if its `modifiedTime` is newer than the last distillation.
...
- If the file hasn't been modified since rejection, it will appear as `"ingested"` (if it was previously ingested) or `"new"` (if never ingested). Re-propose `"new"` files each run — the user can reject again if still not relevant.
...
- **MCP Gateway: list_drive_files** — folder browsing (Mode A, Step 1)
- **MCP Gateway: read_drive_file** — file content reading (Mode A, Step 1)
- **MCP Gateway: get_drive_changes_summary** — change discovery with knowledge cross-reference (Mode A, Step 1)
- **MCP Gateway: query_db** — pipeline state queries (Mode B), project ID resolution (Step 2)
- **MCP Gateway: search_knowledge** — deduplication (Step 4) and connection proposals (Step 5)
- **MCP Gateway: insert_record** — knowledge entry creation (Step 4), task creation (Step 5)
- **MCP Gateway: log_pipeline_run** — run logging (Step 5)
